...
void MissionPage::setupMission1()
{
    ...
    // "REPLAY" button — replay LED signal (below image)
    auto *btnReplay = new QPushButton(QStringLiteral("\u25b6 REPLAY"), leftPanel);
    ...
    btnReplay->setStyleSheet(QStringLiteral(
        ...
        "QPushButton:hover { background-color: #00bfff; color: #0c0c0c; }"
...
void MissionPage::showStoryPopup()
{
    ...
    storyLines
        << QStringLiteral("<span style='color:#666; %1'>[21:04:12]</span>&nbsp;&nbsp;"
                          "<span style='color:#00bfff; %1'>SECURITY SYSTEM v2.1</span>").arg(sf)
        << QStringLiteral("<span style='color:#666; %1'>[21:04:14]</span>&nbsp;&nbsp;"
                          "<span style='color:#ff4444; %1'>[1단계 인증]</span>").arg(sf)
        << QString()
        << QStringLiteral("<span style='color:#00ff41; %1'>비콘 장애 원인을 조사하던 중</span>").arg(sf)
        << QStringLiteral("<span style='color:#00ff41; %1'>손상된 장치에서 마지막 통신 로그가 복구되었습니다.</span>").arg(sf)
        << QStringLiteral("<span style='color:#00ff41; %1'>장애 직전 비콘이 수신한 신호 데이터가 남아있습니다.</span>").arg(sf)
        << QString()
        << QStringLiteral("<span style='color:#eab308; %1'>LED 점멸 신호를 해독하여</span>").arg(sf)
        << QStringLiteral("<span style='color:#eab308; %1'>1단계 인증 코드를 입력하십시오.</span>").arg(sf);
    ...
    // After popup closes, trigger LED
#ifdef Q_OS_LINUX
    led_show_problem();
#endif
}
